...
function Smithers(opts) {
  if (!(this instanceof Smithers)) return new Smithers(opts)
  this.services = []
  this.running = true
...
/**
 * Creates and returns a smithers instance.
 * @param {Object} opts
 * @param {String} opts.root Path to root folder
 */
module.exports = function(opts) {
  return new Smithers(opts)
}
 ...
 */
Smithers.prototype.build = function(opts, cb) {
...
Smithers.prototype.processJobs = function() {
  if (!this.running) return
...
Smithers.prototype.shutdown = function(cb) {
  var that = this
...
Smithers.prototype.doShutdown = function() {
  console.log('Shutting down.')
  this.running = false
  this.shutdownCb()
}

Smithers.prototype.hasRunningJobs = function() {
  return this.services.filter(function(service) {
    return service.jobs.length && service.jobs[0].running
  }).length > 0
}

Smithers.prototype.getService = function(name) {
